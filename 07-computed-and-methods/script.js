Vue.createApp({
  ...
  computed: {
    filteredBooks() {
      const bookRebatePercentage = 20;
      return this.rebatedBooks(bookRebatePercentage).filter((book) => {
        return book.publisher === `O'Reilly Media`;
      });
    },
  },
  methods: {
    rebatedBooks(rebatePercentage) {
      const newBooks = this.books.map((book) => {
        let newPrice = book.price.slice(1) * ((100 - rebatePercentage) / 100);
        newPrice = Number.parseFloat(newPrice).toFixed(2);

        book.price = `${book.price} $${newPrice}`;
        // book.price = `<span style="text-decoration: line-through;">$${book.price}</span> $${newPrice}`;
        return book;
      });
      return newBooks;
    },
